...
import { careTitle, careList, careSubtitle } from '../../fixtures/data'
    ...
    return (
        <div id='care' className='px-10 max-w-[1490px] mx-auto mb-[160px]'>
            ...
            <FadeIn delay={0.4} direction={"down"}>
                <h5 className='text-[#4f4f4f] text-center text-lg xs:text-xl mb-12 font-semibold'>
                    {careSubtitle}
                </h5>
            </FadeIn>
                    ...
                    {careList.map((item, index) => (
                        <FadeIn key={index} delay={(index + 1) * 0.2} direction={"left"}>
                            <div className='flex flex-col xs:flex-row gap-6 items-center xs:items-start'>
                                <img src={item.img} alt={item.title} className='h-[68px] w-[68px]' />
                            </div>
                            <div>
